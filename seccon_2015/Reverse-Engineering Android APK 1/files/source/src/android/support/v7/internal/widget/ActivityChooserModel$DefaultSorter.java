    ...
    implements 
{
    ...
    public void sort(Intent intent, List list, List list1)
    {
        ...
        for (int i = 0; i < k; i++)
        {
            ...
            einfo.weight = 0.0F;
            intent.put(new ComponentName(einfo.resolveInfo.activityInfo.packageName, einfo.resolveInfo.activityInfo.name), einfo);
        }
        ...
        for (j--; j >= 0;)
        {
            ...
            eInfo einfo1 = (eInfo)intent.get(rd.activity);
            float f1 = f;
            if (einfo1 != null)
            {
